...
export class ModifyProjectMembers1717961669938 implements MigrationInterface {
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        `)
        for (const projectMember of projectMembers) {
            const users = await queryRunner.query('SELECT * FROM "user" WHERE id = $1', [projectMember.userId])
            const user = users[0]
            await queryRunner.query(`
            INSERT INTO "project_member" ("id", "created", "updated", "projectId", "platformId", "email", "status", "role")
            VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
        `, [projectMember.id, dayjs(projectMember.created).toISOString(), dayjs(projectMember.updated).toISOString(), projectMember.projectId, projectMember.platformId, user.email, projectMember.status, projectMember.role])
        }
    }
