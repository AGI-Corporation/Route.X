    ...
    app.get(
        ...
        async (
            ...
            if (!params.code) {
                ...
                const targetOrigin = await domainHelper.getPublicUrl({ platformId })
                const expectedOrigin = new URL(targetOrigin).origin
                return reply
                    .type('text/html')
                    .send(
                        `<html><body><script>if(window.opener){window.opener.postMessage({ 'code': '${encodeURIComponent(
                            params.code,
                        )}' }, '${expectedOrigin}')}</script> Redirect successfully, this window should close now</body></html>`,
                    )
            }
        },
    )
